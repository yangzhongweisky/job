...
function wppShowMenuPopup(objMenu, popupId)
{
    ...
    var popup = $('#' + popupId);
    var totalwidth=0;
    popup.find('.column').each(
            function(index,domEle){ totalwidth +=$(domEle).width()}
        );
    if(totalwidth<630){
        totalwidth+=40;
        popup.find('.full_block').attr("style","width:"+totalwidth+"px");
    }
    if (!popup)
        return;
    popup.show();
    objMenu.addClass('actiwe');
	...
	popup.css('top', 0 + 'px');
    popup.css('left', '220px');
    if (CUSTOMMENU_POPUP_WIDTH)
        popup.css('width', CUSTOMMENU_POPUP_WIDTH + 'px');
}
function wppPopupPos(objMenu, w)
{
    ...
    var wWraper = wraper.width() - CUSTOMMENU_POPUP_RIGHT_OFFSET_MIN;
...
function wppHideMenuPopup(objMenu, event, popupId, menuId)
{
    objMenu = $(objMenu);
    var popup = $('#' + popupId);
    if (!popup)
    ...
    if (!($(current_mouse_target).closest(objMenu).length > 0 || $(current_mouse_target).closest(popup).length>0))
    {
            popup.hide();
            $('#' + menuId).removeClass('actiwe');
    }
}
